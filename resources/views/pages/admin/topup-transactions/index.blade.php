...
<div class="card-body">
    <div class="table-responsive">
        <table id="topup-transactions" class="table table-bordered table-hover table-striped" width="100%" cellspacing="0">
            <thead>
                <tr>
                    <th>Id</th>
                    <th>Select</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#topup-transactions').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url('/admin/topup-transactions') }}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
        ...
        $('body').on('click', '.edit', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('/admin/topup-transactions/edit') }}",
                data: {
                    id: id
                },
                dataType: 'json',
        ...
        $('body').on('click', '.delete', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ url('/admin/topup-transactions/delete') }}",
                        data: {
                            data: id,
                            multi: null
                        },
                        dataType: 'json',
                        success: function(res) {
                            var oTable = $('#topup-transactions').dataTable();
                            oTable.fnDraw(false);
        ...
        $('body').delegate('#addEditTransForm', 'submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ url('/admin/topup-transactions/store')}}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
